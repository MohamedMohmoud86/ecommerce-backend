...
function Cart() {
    ...
    const handleCheckout = async () => {
        try {
            ...
            const currentUserId = savedUser?.id || savedUser?.uid || savedUser?._id;
            const orderRes = await axios.post(
                "http://192.168.1.13:5000/api/create-order",
                {
            ...
            const paymentRes = await axios.post(
                "http://192.168.1.13:5000/api/payment",
                {
                    amount: Number(total),
                    ...
                    walletIdentifier: paymentMethod === "wallet" ? walletNumber : "" 
                }
            );
                                ...
                                <div className="item_cart" key={index}>
                                    <div className="image_name">
                                        <div className='img_item'>
                                            <img src={item.thumbnail || (item.images && item.images.length > 0 ? item.images[0] : "https://via.placeholder.com/150")} alt={item.title} />
                                        </div>
                                        ...
                                        <div className="content">
                                            ...
                                            <div className="quantity_control">
                                                ...
                                                <button onClick={() => increaseQuantity(item._id || item.id)}>+</button>
                                            </div>
                                        </div>
                                    </div>
                                    <button onClick={() => removeFromCart(item._id || item.id)} className='delete_item'><FaTrashAlt /></button>
                                </div>
                            ))
                        )}
                    </div>
